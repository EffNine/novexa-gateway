...
- **Model Online Status** — Optional probes hide unreachable models (especially NVIDIA NIM); startup pass then once per day by default, with status on `/api/models` and `/api/models/status`
